...
public class LzopInputStream extends BlockDecompressorStream
{
    private static final Log                  LOG                   = LogFactory
                                                                            .getLog(LzopInputStream.class);

    private final EnumSet<DChecksum>          dflags                = EnumSet
                                                                            .allOf(DChecksum.class);
    private final EnumSet<CChecksum>          cflags                = EnumSet
                                                                            .allOf(CChecksum.class);

    private final byte[]                      buf                   = new byte[9];
    private final EnumMap<DChecksum, Integer> dcheck                = new EnumMap<DChecksum, Integer>(
                                                                            DChecksum.class);
    private final EnumMap<CChecksum, Integer> ccheck                = new EnumMap<CChecksum, Integer>(
                                                                            CChecksum.class);
    ...
    public void readHeader(InputStream in) throws IOException
    {
        ...
        if (hitem < LzoDecompressor.MINIMUM_LZO_VERSION)
        {
            throw new IOException(
                    "Compressed with incompatible lzo version: 0x"
                            ...
                            + " (expected at least 0x"
                            + Integer
                                    .toHexString(LzoDecompressor.MINIMUM_LZO_VERSION)
                            + ")");
        }
        ...
        try
        {
            for (DChecksum f : dflags)
            {
                ...
                {
                    dcheck.put(f, (int) f.getChecksumClass().newInstance()
                            .getValue());
                }
            }
            for (CChecksum f : cflags)
            {
                ...
                {
                    ccheck.put(f, (int) f.getChecksumClass().newInstance()
                            .getValue());
                }
            }
        }
        ...
        { // lzop 1.08 ultimately ignores this
            System.out.println("Extra header field not processed");
            adler.reset();
            crc32.reset();
            hitem = readHeaderItem(in, buf, 4, adler, crc32);
            readHeaderItem(in, new byte[hitem], hitem, adler, crc32);
    ...
    private void verifyChecksums() throws IOException
    {
        ...
        for (Map.Entry<CChecksum, Integer> chk : this.ccheck.entrySet())
        {
    ...
    @Override
    protected/* int */void getCompressedData() throws IOException
    {
        this.checkStream();
        ...
        if (compressedLen > LzopCodec.MAX_BLOCK_SIZE)
        {
            throw new IOException("Compressed length " + compressedLen
                    + " exceeds max block size " + LzopCodec.MAX_BLOCK_SIZE
                    + " (probably corrupt file)");
        }

        LzopDecompressor ldecompressor = (LzopDecompressor) this.decompressor;
        ldecompressor
                .setCurrentBlockUncompressed(compressedLen >= this.uncompressedBlockSize);
    ...
    public void close() throws IOException
    {
        ...
        {
            LOG.warn(
                    "Incorrect LZO file format: file did not end with four trailing zeroes.",
                    ex);
        }
    }
}
